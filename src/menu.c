...
const cfg_t *list[] = { cfg };

void CFG_Sync(void) {
  CFG_update_config("/persistent/doom2d.cfg", "/persistent/doom2d.cfg", 4, list, "generated by doom2d, do not modify");
  sync_fs_quit();
}
#endif
#define PCOLORN 10
...
#ifdef __EMSCRIPTEN__
        // skip due to bug where the game won't quit        
#else
        S_wait();
#endif
        ERR_quit();
...
int GM_act (void) {
  ...
  if (m == NULL) {
#if defined(__EMSCRIPTEN__)
    if (lastkey == KEY_GRAVE || (state_for_anykey(g_st) && lastkey != KEY_UNKNOWN)) {
#else
